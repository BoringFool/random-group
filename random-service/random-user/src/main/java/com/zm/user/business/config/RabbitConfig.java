package com.zm.user.business.config;
...
public class RabbitConfig {
    ...
    @Autowired
    public RabbitConfig(ConnectionFactory rabbitConnectionFactory,  RabbitProperties properties){
...
//    }
    ...
    public RabbitTemplate rabbitTemplate(){
        ...
        rabbitTemplate.setConfirmCallback(
                (correlationData, b, s) -> {
                     logger.warn("ConfirmCallback     "+"相关数据：{}", correlationData);
                     logger.warn("ConfirmCallback     "+"确认情况：{}", b);
                     logger.warn("ConfirmCallback     "+"原因：{}", s);
                });
        //Message
        rabbitTemplate.setReturnsCallback(returnedMessage -> {
            logger.warn("ReturnCallback：     "+"消息：{}", returnedMessage.getMessage());
            logger.warn("ReturnCallback：     "+"回应码：{}", returnedMessage.getReplyCode());
            logger.warn("ReturnCallback：     "+"回应消息：{}", returnedMessage.getReplyText());
            logger.warn("ReturnCallback：     "+"交换机：{}", returnedMessage.getExchange());
            logger.warn("ReturnCallback：     "+"路由键：{}", returnedMessage.getRoutingKey());
        });
    ...
    public RetryTemplate rabbitRetryTemplate() {
        ...
        retryTemplate.registerListener(new RetryListener() {
            ...
            public <T, E extends Throwable> void onError(RetryContext retryContext, RetryCallback<T, E> retryCallback, Throwable throwable) {
                //  异常 都会调用
                logger.warn("-----第{}次调用", retryContext.getRetryCount());
            }
        });
    ...
    public Queue directQueue() {
        ...
        return new Queue("test.queue", true, false, false, argsMap);
    }

    //Direct交换机
    @Bean
    DirectExchange directExchange() {
        return new DirectExchange("test.directExchange", true, false);
    }

    //绑定
    @Bean
    Binding bindingDirect() {
        return BindingBuilder.bind(directQueue()).to(directExchange()).with("test.directRouting");
    }
}
